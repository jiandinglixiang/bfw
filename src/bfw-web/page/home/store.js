    ...
    switch (action.type) {
      ...
      case setKindData().type:
        if (Array.isArray(action.endMatchList)) {
          data.endMatchList = f1(state.endMatchList, action.endMatchList, action.kindId, action.time)
        }
        if (Array.isArray(action.endMatchList)) {
          data.notStartMatchList = f1(state.notStartMatchList, action.notStartMatchList, action.kindId, action.time)
        }
        if (Array.isArray(action.endMatchList)) {
          data.startMatchList = f1(state.startMatchList, action.startMatchList, action.kindId, action.time)
        }
        if (Array.isArray(action.endMatchList)) {
          data.totalNumber = f1(state.totalNumber, action.totalNumber, action.kindId, action.time)
        }
        data.time = action.time
        data.kindId = action.kindId
        return Object.assign({}, state, data)
